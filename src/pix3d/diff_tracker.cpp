...
std::tuple<VecX, MatX> DiffTracker::ComputeResidualAndJacobian(
    ...
    _timer.Tick("SetMesh");
    _engine->SetMesh(V, _F);
    _timer.Tock("SetMesh");

    _timer.Tick("Render");
    std::vector<EdgePixel> edgelist;
    _engine->ComputeEdgePixels(Mat4f::Identity(), edgelist);
    _timer.Tock("Render");
...
cv::Mat DiffTracker::RenderEstimate() const {
    ...
    _engine->RenderDepth(Mat4::Identity(), depth);
    return depth;
}
...
cv::Mat DiffTracker::RenderEdgepixels() const {
    std::vector<EdgePixel> edgelist;
    _engine->ComputeEdgePixels(Mat4::Identity(), edgelist);
