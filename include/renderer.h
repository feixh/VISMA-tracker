...
public:
    ...
    void SetCamera(const Mat4f &pose);
    ...
    void SetMesh(const std::vector<float> &vertices, const std::vector<int> &faces) {
        SetMesh(const_cast<float*>(&vertices[0]), vertices.size() / 3, const_cast<int*>(&faces[0]), faces.size()/3);
    }

    /// \brief: Set parameter for one-dimensional search.
//    void SetOneDimSearch(const Options &opt);
    ...
    void RenderDepth(const Mat4f &model, float *out);
    void RenderDepth(const Mat4f &model, cv::Mat &depth_map) {
    ...
    void RenderEdge(const Mat4f &model, uint8_t *out);
    void RenderEdge(const Mat4f &model, cv::Mat &edge_map) {
    ...
    void RenderMask(const Mat4f &model, uint8_t *out);
    void RenderMask(const Mat4f &model, cv::Mat &mask) {
    ...
    void ComputeEdgePixels(const Mat4f &model, std::vector<EdgePixel> &edgelist);
//    void OneDimSearch(const Mat4f &model, std::array<float, 6> &out);
    void OneDimSearch(const Mat4f &model, std::vector<EdgePixel> &edgelist);

    void RenderWireframe(const Mat4f &model, uint8_t *out);
    void RenderWireframe(const Mat4f &model, cv::Mat &wireframe) {
